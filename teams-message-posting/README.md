...
構成は非常に単純で下図を参照のこと。また、メンション先の人数は「ユーザーの＠mention トークンを取得する」の数と一致する。（今回は特定の2人にメンションを想定しているため、同じアクションが二つある）。また、アクション名はフロー作成時、アクション名を探しやすいことを考慮し、名前の変更は加えていない。
...
自分の作成したworkflowが問題なく動くことを確認したら、［チャットまたはチャネルでメッセージを投稿する］アクションの適用先を変更し、今回作成したワークフローを活用しよう。
